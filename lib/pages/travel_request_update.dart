...
class TravelRequestUpdate extends StatelessWidget {
  ...
  String purpose;
  String destination;
  TravelRequestListResponse travelRequest;
  @override
  Widget build(BuildContext context) {
     travelRequest = Get.arguments;
       controller.setTravelRequestListResponse(travelRequest);
   if(travelRequest !=null){
     selectedFromDate = DateTime.tryParse(controller.fromDateTextController.text);
     selectedToDate = DateTime.tryParse(controller.toDateTextController.text);
   }
        ...
        child: SingleChildScrollView(
          child: Column(
            children: [
              ...
              GetBuilder<TravelRequestUpdateController>(
                   builder: (controller) => controller.is_show_auto_generate.value
                    ? GFButton(
                  onPressed: () {
                    showDialog(
                        context: context,
              ...
              GetBuilder<TravelRequestUpdateController>(
                  builder:(controller) => controller.expenseList.length > 0
                          ...
                          if (controller.checkIsNotEmptyPurpose()) {
                            if (controller.expenseList.length == 0) {
                              ...
                              if (_formKeyParent.currentState.validate()) {
                                controller.updateTravelRequest(
  ...
  showAlertDialog(BuildContext context) {
    var message = "Are you sure to save?There is no advance!";
    ...
    Widget continueButton = FlatButton(
      ...
      onPressed: () {
        Navigator.of(context).pop();
        controller.updateTravelRequest(controller.durationController.text,travelRequest.id);
      },
    ); //
                    ...
                    Expanded(
                      ...
                      child: Container(
                        ...
                        child: TextField(
                          ...
                          onChanged: (text) {
                            controller.expenseList[index].remark = text;
  ...
  Future<Null> _selectDate(BuildContext context, String date) async {
    ...
    if (picked != null) {
      if (date == 'From Date') {
        ...
        // final to_time = await showTimePicker(
        //   context: context,
        //   initialTime: TimeOfDay.fromDateTime(DateTime.now()),
        // );
        // var to_date_time =
        //     DateTimeField.combine(picked, to_time).toString().split('.000')[0];
        // print(to_date_time);
        // selectedToDate = DateTime.parse(to_date_time);
